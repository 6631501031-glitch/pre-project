...
| response | legacy modules often use settings message response service; `graduationsystemusingfacerecognition` routes use direct `{ code, message, data }` JSON |
...
| Mount | Module |
|---|---|
| `/api/v1/graduationsystemusingfacerecognition` | `Project/graduationsystemusingfacerecognition/graduationsystemusingfacerecognition.routes.js` |
...
Note: `frontend-vue/src/service/api.js` currently calls the document API with lowercase `/api/v1/graduationsystemusingfacerecognition`. Verify Express case-sensitivity and preserve compatibility before changing route casing.
...
- document registry mutation: `edit/delete` on `/graduation-system-using-face-recognition/registry`
...
```txt
ทำหน้าที่ Backend Agent สำหรับ GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION
