# IAM System Overview — GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Backend Node.js

> เอกสารนี้สรุปสถาปัตยกรรม วิธีการทำงาน รูปแบบการเขียนโค้ด และข้อกำหนดของระบบ IAM ใน GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Backend
...
GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Backend ใช้ระบบ IAM (Identity and Access Management) แบบ **Dual Permission Model** ซึ่งสามารถทำงานได้ 2 โหมด:
...
| **Local Fallback** | `PROJECT_PERMISSION_SOURCE=local` | ใช้ฐานข้อมูล MongoDB ของ GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION เอง |
...
- รองรับ B2B Token สำหรับ GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION API
...
```
┌─────────────────────────────────────────────────────────┐
│                   GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Backend                        │
│                                                          │
│  ┌──────────┐    ┌──────────────┐    ┌───────────────┐  │
...
|-----------|------|---------|
| IAM SDK | `server/integrations/iam/sdk.js` | Factory สร้าง SDK instance สำหรับ GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION |
...
| B2B Auth | `graduationsystemusingfacerecognition/service/graduationsystemusingfacerecognition_document.js` | ตรวจสอบ B2B Token สำหรับ GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION API |
...
- สร้าง GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION-configured IAM SDK instance ด้วย `createGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONSdk()`
...
| `forwardAccountsList()` | List accounts พร้อม GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION scope filter |
| `resolveScopedSecurityMetadata()` | กรอง Groups/Menus ตาม GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION scope |
| `inviteAccountToScope()` | เชิญ user ใหม่เข้า GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION scope |
| `syncAccountAssignments()` | Sync group memberships |
| `removeAccountFromScope()` | ปิดการเข้าถึง GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION ทั้งหมดของ account |
...
```
Client → GET /api/v1/graduationsystemusingfacerecognition/documents/...
  → onCheckAuthorization() → ดึง account จาก x-access-token
  → requirePermission('/graduation-system-using-face-recognition/registry', 'view')
    → normalizePath('/graduation-system-using-face-recognition/registry')
    ...
    → ตรวจสอบ matrix['/graduation-system-using-face-recognition/registry']['view'] === true
  → ถ้าผ่าน: next()
...
### B2B Token Flow
```
External System → POST /api/v1/graduationsystemusingfacerecognition/documents/...
  + Authorization: Bearer <token>
  → onCheckB2BAuthorization()
    → introspectToken() → IAM Service
    → ตรวจสอบ active, audience, scope
  → requireScope('graduation.system.using.face.recognition.registry.write')
    → ตรวจ token.scopes ประกอบด้วย 'graduation.system.using.face.recognition.registry.write'
  → ถ้าผ่าน: next()
```
...
| `IAM_SDK_CLIENT_ID` | `graduationsystemusingfacerecognition-api` | Client ID สำหรับ IAM |
...
| `PROJECT_PERMISSION_TYPE_TITLE` | `GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Administration` | ชื่อ type ใน IAM |
...
```
graduation.system.using.face.recognition.registry.read graduation.system.using.face.recognition.registry.write graduation.system.using.face.recognition.report.read
