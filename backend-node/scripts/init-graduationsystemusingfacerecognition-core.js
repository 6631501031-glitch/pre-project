...
const projectName = project.name || process.env.PROJECT_NAME || 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION';
const projectCode = String(project.code || process.env.PROJECT_CODE || 'graduationsystemusingfacerecognition')
  .trim()
  .toLowerCase()
  .replace(/[^a-z0-9]+/g, '-')
  .replace(/^-+|-+$/g, '') || 'graduationsystemusingfacerecognition';
