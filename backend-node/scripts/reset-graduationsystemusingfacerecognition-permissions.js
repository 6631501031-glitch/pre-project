...
const DEFAULT_TYPE_TITLE = securityConfig.permissionTypeTitle || 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Administration';
const DEFAULT_GROUP_TITLE = securityConfig.permissionGroupTitle || 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Admin';
...
const DEFAULT_MENU_PATHS = (
  Array.isArray(securityConfig.permissionPaths) && securityConfig.permissionPaths.length > 1
    ...
    : [
  securityConfig.permissionRootPath || '/graduation-system-using-face-recognition/security/permission',
  '/dashboard',
...
function ensureConfig() {
  if (!graduationsystemusingfacerecognitionConfig.iamAdmin.baseUrl || !graduationsystemusingfacerecognitionConfig.iamAdmin.clientId || !graduationsystemusingfacerecognitionConfig.iamAdmin.clientSecret) {
...
async function getAdminToken() {
  ...
  const response = await axios.post(
    `${graduationsystemusingfacerecognitionConfig.iamAdmin.baseUrl}${graduationsystemusingfacerecognitionConfig.iamAdmin.tokenPath}`,
    {
      grant_type: 'client_credentials',
      client_id: graduationsystemusingfacerecognitionConfig.iamAdmin.clientId,
      client_secret: graduationsystemusingfacerecognitionConfig.iamAdmin.clientSecret,
      scope: graduationsystemusingfacerecognitionConfig.iamAdmin.scope,
      audience: graduationsystemusingfacerecognitionConfig.iamAdmin.audience
    },
    {
      timeout: graduationsystemusingfacerecognitionConfig.iamAdmin.timeout
    }
  );
...
async function adminRequest(token, method, path, payload, params) {
  try {
    ...
    const requestConfig = {
      method: method,
      url: `${graduationsystemusingfacerecognitionConfig.iamAdmin.baseUrl}${graduationsystemusingfacerecognitionConfig.iamAdmin.basePath}${path}`,
      headers: {
...
async function ensureType(token) {
  const created = await adminRequest(token, 'post', '/security/type', {
    ...
    description: toLang('GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION delegated administration type')
  });
  return created.data;
}
...
async function ensureGroup(token, typeId) {
  const created = await adminRequest(token, 'post', '/security/group', {
    ...
    description: toLang('GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION delegated administration group'),
    visibleType: typeId,
    state: true
  });
  return created.data;
...
async function ensureMenus(token, typeId) {
  ...
  for (const path of DEFAULT_MENU_PATHS) {
    const created = await adminRequest(token, 'post', '/security/menu', {
      title: toLang(path),
      description: toLang(`GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION permission for ${path}`),
      path: path,
      type: typeId,
      state: true
    });
