...
function createUserRequestOptions(request, options) {
  ...
  return {
    ...
    headers: {
      ...
      'x-system': config.project && config.project.code ? String(config.project.code) : 'graduationsystemusingfacerecognition'
    }
  };
}
...
async function loadCurrentGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccount(request) {
...
async function loadGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountById(accountId) {
  if (!accountId) return null;
  try {
...
async function resolveIamAccountIdByEmail(email) {
  if (!email) return '';
  ensureAdminConfig();
  const iamSdk = createGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONSdk();
...
async function resolveIamAccountIdByGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountId(accountId) {
  const graduationsystemusingfacerecognitionAccount = await loadGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountById(accountId);
  ...
  try {
    ...
    if (!allowed) {
      ...
      return response.status(403).json({
        status: false,
        error: 'account_not_in_graduationsystemusingfacerecognition_scope'
      });
    }
...
async function inviteAccountToScope(payload, headers) {
  ...
  const groupIds = Array.isArray(body.groupIds)
    ...
    : [];
  if (!groupIds.length) {
    const error = new Error('graduationsystemusingfacerecognition_group_required');
    error.statusCode = 400;
    error.payload = { status: false, error: 'graduationsystemusingfacerecognition_group_required' };
    throw error;
  }
...
module.exports = {
  ...
  resolveIamAccountIdByGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountId,
  requestAdmin,
  requestUser,
  forwardAdminRequest,
