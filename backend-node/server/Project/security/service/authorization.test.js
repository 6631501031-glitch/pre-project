...
test.before(async function () {
  ...
  config.iamAdmin.scope = 'graduation.system.using.face.recognition.registry.read graduation.system.using.face.recognition.registry.write graduation.system.using.face.recognition.report.read iam.security.read iam.security.write iam.audit.read iam.accounts.read';
  ...
  AccountModel.onQuery = async function (query) {
    if (query && String(query._id) === 'graduationsystemusingfacerecognition-account-1') {
      return {
...
test('account-access resolves effective permissions from IAM', async function () {
  ...
  const result = await accountAccess.getEffectivePermissions('graduationsystemusingfacerecognition-account-1');
...
test('account-access resolves IAM group options and assignment snapshots', async function () {
  ...
  const assignmentsByAccountId = await accountAccess.loadAssignmentsByAccountIds(['graduationsystemusingfacerecognition-account-1']);

  assert.equal(groups.length >= 2, true);
  assert.equal(groups[0]._id, 'group-1');
  assert.equal(Array.isArray(assignmentsByAccountId['graduationsystemusingfacerecognition-account-1']), true);
  assert.equal(assignmentsByAccountId['graduationsystemusingfacerecognition-account-1'][0].group._id, 'group-1');
});
