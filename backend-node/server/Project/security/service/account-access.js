...
async function findGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountById(accountId) {
...
async function resolveIamAccountIdByGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountId(accountId) {
  const graduationsystemusingfacerecognitionAccount = await findGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountById(accountId);
  if (!graduationsystemusingfacerecognitionAccount) return String(accountId || '');
  const email = graduationsystemusingfacerecognitionAccount && graduationsystemusingfacerecognitionAccount.email ? String(graduationsystemusingfacerecognitionAccount.email).trim().toLowerCase() : '';
  if (!email) return String(accountId || '');
  const sdk = createGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONSdk();
...
async function getIamPermissionData(accountId, options) {
  ...
  const iamAccountId = await resolveIamAccountIdByGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountId(accountId);
  ...
  if (options && options.targetAccountId) {
    targetIamAccountId = await resolveIamAccountIdByGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountId(options.targetAccountId);
...
async function loadAssignmentsByAccountIds(accountIds) {
  ...
  if (useIamPermissionSource()) {
    const sdk = createGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONSdk();
    const rows = await Promise.all(ids.map(async function (graduationsystemusingfacerecognitionAccountId) {
      try {
        const iamAccountId = await resolveIamAccountIdByGRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONAccountId(graduationsystemusingfacerecognitionAccountId);
        if (!iamAccountId) {
          return [String(graduationsystemusingfacerecognitionAccountId), []];
        }
        ...
        return [String(graduationsystemusingfacerecognitionAccountId), assignments];
      } catch (error) {
        return [String(graduationsystemusingfacerecognitionAccountId), null];
      }
    }));
