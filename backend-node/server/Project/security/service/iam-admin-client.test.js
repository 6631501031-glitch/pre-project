...
function setScopedGroups() {
  mockServer.state.groups = [
    ...
    },
    {
      ...
      visibleType: { _id: 'type-graduationsystemusingfacerecognition', title: [{ key: 'en', value: 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Administration' }] }
    },
    {
      _id: 'group-3',
      title: [{ key: 'en', value: 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Operator' }],
      visibleType: { _id: 'type-graduationsystemusingfacerecognition', title: [{ key: 'en', value: 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Administration' }] }
    }
  ];
}
...
function setScopedMenus() {
  mockServer.state.menus = [
    ...
    },
    {
      ...
      type: { _id: 'type-graduationsystemusingfacerecognition', title: [{ key: 'en', value: 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Administration' }] }
    },
    {
...
function setScopedPermissions() {
  mockServer.state.permissions = [
    ...
    {
      _id: 'perm-2',
      name: 'graduationsystemusingfacerecognition.accounts.manage',
    ...
    {
      _id: 'perm-3',
      name: 'graduationsystemusingfacerecognition.lifecycle.manage',
      group: { _id: 'group-2', name: 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Admin' },
...
function resetMockState() {
  ...
  mockServer.state.accounts = [{ _id: 'acc-1', email: 'graduationsystemusingfacerecognition.ops@example.com' }];
...
test.before(async function () {
  ...
  config.iam.requiredAudience = 'graduationsystemusingfacerecognition-api';
  config.iamAdmin.baseUrl = baseUrl;
  config.iamAdmin.tokenPath = '/api/v1/b2b/token';
  config.iamAdmin.basePath = '/api/v1/b2b/admin';
  config.iamAdmin.clientId = 'graduation-system-using-face-recognition-sdk';
  config.iamAdmin.clientSecret = 'super-secret';
  config.iamAdmin.scope = 'iam.security.write iam.audit.read iam.accounts.read';
  config.iamAdmin.audience = 'graduationsystemusingfacerecognition-api';
  originalPermissionTypeTitle = config.security.permissionTypeTitle;
  config.security.permissionTypeTitle = 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Administration';
  ...
  AccountModel.onQuery = async function (query) {
    ...
    if (accountId === 'graduationsystemusingfacerecognition-account-1') {
      return {
...
test('iam-admin-client resolves my permissions from graduationsystemusingfacerecognition account email', async function () {
  ...
  mockServer.state.permissions.push({
    _id: 'perm-config',
    name: 'graduationsystemusingfacerecognition.auth-message.manage',
    group: { _id: 'group-2', name: 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Admin' },
  ...
  mockServer.state.permissions.push({
    _id: 'perm-training',
    name: 'graduationsystemusingfacerecognition.legacy.manage',
    group: { _id: 'group-2', name: 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Admin' },
  ...
  await iamAdminClient.forwardMyPermissions({
    body: { accounts: 'graduationsystemusingfacerecognition-account-1' },
    query: {},
    headers: { lang: 'en' }
  }, response);
  ...
  assert.deepEqual(
    ...
    ['graduationsystemusingfacerecognition.accounts.manage', 'graduationsystemusingfacerecognition.auth-message.manage', 'graduationsystemusingfacerecognition.lifecycle.manage']
  );
  assert.deepEqual(
...
test('iam-admin-client resolves current account and proxies IAM user requests', async function () {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(response.payload.data.email, 'graduationsystemusingfacerecognition.ops@example.com');
});

test('iam-admin-client allows scoped signin only for accounts inside graduationsystemusingfacerecognition scope', async function () {
  ...
  assert.equal(deniedResponse.payload.error, 'account_not_in_graduationsystemusingfacerecognition_scope');
...
test('iam-admin-client forwards account list and effective permissions', async function () {
  ...
  assert.deepEqual(
    ...
    ['graduationsystemusingfacerecognition.accounts.manage', 'graduationsystemusingfacerecognition.lifecycle.manage']
  );
  assert.deepEqual(
...
test('iam-admin-client lists all graduationsystemusingfacerecognition scoped accounts by default and supports requested pagination', async function () {
  config.security.permissionTypeTitle = 'GRADUATION SYSTEM USING FACE RECOGNITION GRADUATION SYSTEM USING FACE RECOGNITION Administration';
  mockServer.state.accounts = [
...
test('iam-admin-client filters account group options to graduationsystemusingfacerecognition permission type', async function () {
...
test('iam-admin-client syncs only graduationsystemusingfacerecognition scoped account assignments to IAM', async function () {
  mockServer.state.accountAssignments = [
    {
      _id: 'acct-assign-1',
      account: 'acc-1',
  ...
  const result = await iamAdminClient.syncAccountAssignments('graduationsystemusingfacerecognition-account-1', ['group-1', 'group-2']);
...
test('iam-admin-client removes only graduationsystemusingfacerecognition scoped account assignments', async function () {
  mockServer.state.accountAssignments = [
    {
      _id: 'acct-assign-graduationsystemusingfacerecognition',
      account: { _id: 'acc-1', email: 'graduationsystemusingfacerecognition.ops@example.com' },
...
test('iam-admin-client enforces scope guard before account mutations', async function () {
  ...
  assert.equal(denyResponse.payload.error, 'account_not_in_graduationsystemusingfacerecognition_scope');
});
...
test('iam-admin-client invites accounts into graduationsystemusingfacerecognition scope only', async function () {
  mockServer.state.accounts = [{ _id: 'acc-1', email: 'graduationsystemusingfacerecognition.ops@example.com' }];
  mockServer.state.accountAssignments = [];
  ...
  assert.equal(invited.account.email, 'new.graduationsystemusingfacerecognition@example.com');
  assert.deepEqual(
  ...
  await iamAdminClient.forwardInviteAccountToScope({
    headers: { lang: 'en' },
    body: {
      email: 'existing.graduationsystemusingfacerecognition@example.com',
      groupIds: ['group-2']
    }
  }, response);
