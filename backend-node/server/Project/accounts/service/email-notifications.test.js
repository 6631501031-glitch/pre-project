...
test('email notification templates support env overrides and placeholders', function () {
    withEnv({
        PROJECT_MAIL_APP_NAME: 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Test',
        PROJECT_APP_URL: 'https://graduationsystemusingfacerecognition.example.test',
        ...
        assert.equal(template.subject, 'Invite user@example.test to GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Test');
        assert.equal(template.text, 'Hello Jane Doe https://graduationsystemusingfacerecognition.example.test');
        assert.equal(template.html, '<p>Jane Doe</p><a href="https://graduationsystemusingfacerecognition.example.test">user@example.test</a>');
    });
});
...
test('GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION email workflow dispatch calls IAM with graduationsystemusingfacerecognition app scope', async function () {
    ...
    const result = await EmailNotifications.dispatchEmailWorkflow({
        eventKey: 'graduationsystemusingfacerecognition.expiring',
        to: 'customer@example.test',
        context: {
            graduationsystemusingfacerecognitionNo: 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION-1'
        },
        request: {
            headers: {
                lang: 'en'
