...
function payloadFromBody(body) {
  const payload = {
    graduationsystemusingfacerecognitionNo: cleanText(body.graduationsystemusingfacerecognitionNo),
...
function buildListQuery(query) {
  ...
  if (q) {
    filter.$or = [
      { graduationsystemusingfacerecognitionNo: new RegExp(q, 'i') },
...
exports.list = async function list(query) {
  ...
  const [rows, total] = await Promise.all([
    GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.find(filter).sort({ expiryDate: 1, updatedAt: -1 }).skip(skip).limit(limit).lean(),
    GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.countDocuments(filter)
  ]);
...
exports.stats = async function stats() {
  ...
  const [total, active, review, expiring, expired] = await Promise.all([
    GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.countDocuments({}),
    GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.countDocuments({ status: 'active' }),
    GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.countDocuments({ status: 'review' }),
    GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.countDocuments({
      $or: [
        { status: 'expiring' },
...
exports.create = async function create(body, request) {
  ...
  const missing = ['graduationsystemusingfacerecognitionNo', 'title', 'partnerName'].filter(function (field) {
  ...
  const created = await GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.create(updateLifecycleStatus(payload));
  return created.toObject();
};

exports.update = async function update(id, body, request) {
  if (!mongoose.Types.ObjectId.isValid(id)) {
    const error = new Error('Invalid GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION document id');
    error.status = 400;
    throw error;
  }
  ...
  const updated = await GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.findOneAndUpdate(
  ...
  if (!updated) {
    const error = new Error('GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION document not found');
    error.status = 404;
    throw error;
  }
  return updated;
...
exports.remove = async function remove(id) {
  ...
  const result = await GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.deleteOne({ _id: new mongoose.Types.ObjectId(id) });
  return { deleted: result.deletedCount || 0 };
};

exports.seedDemo = async function seedDemo(request) {
  const existing = await GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.countDocuments({});
  if (existing > 0) {
  ...
  await GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITIONDocument.insertMany([
    {
      graduationsystemusingfacerecognitionNo: 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION-INT-2026-001',
      ...
      coordinatorName: 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Coordinator',
      coordinatorEmail: 'graduationsystemusingfacerecognition@mfu.ac.th',
      status: 'active',
      effectiveDate: today,
      expiryDate: monthsFromNow(30),
      renewalNoticeDate: monthsFromNow(27),
      tags: ['academic', 'exchange'],
      notes: 'Initial demo record for validating GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION workflows.',
      create: actor
    },
    {
      graduationsystemusingfacerecognitionNo: 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION-RES-2026-002',
    ...
    {
      graduationsystemusingfacerecognitionNo: 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION-EXP-2026-003',
