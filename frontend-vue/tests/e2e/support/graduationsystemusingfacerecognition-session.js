function bootstrapAuthenticatedRoute(browser, options) {
  var settings = Object.assign({
    route: '/dashboard',
    token: 'graduationsystemusingfacerecognition-e2e-token',
    profile: {
      _id: 'acc-1',
  ...
  return browser.executeAsync(function (bootstrapOptions, done) {
    ...
    function installDispatchShim(app, shimState) {
      ...
      app.$store.__graduationsystemusingfacerecognitionE2EState = Object.assign({}, shimState || {});

      if (app.$store.__graduationsystemusingfacerecognitionOriginalDispatch) {
        return;
      }

      app.$store.__graduationsystemusingfacerecognitionOriginalDispatch = app.$store.dispatch.bind(app.$store);
      app.$store.dispatch = function (type, payload) {
        var currentShimState = this.__graduationsystemusingfacerecognitionE2EState || {};
        ...
        return this.__graduationsystemusingfacerecognitionOriginalDispatch(type, payload);
      };
    }
