...
module.exports = {
  'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION security pages load IAM-backed data and save permission matrix rows': function (browser) {
  ...
  'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION security menu page keeps newly-created scoped types and menus visible': function (browser) {
    const devServer = process.env.VUE_DEV_SERVER_URL
    ...
    browser
      ...
      .executeAsync(function (done) {
        ...
        app.$store.dispatch('security/menu/createType', {
          title: [{ key: 'en', value: 'Custom GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Tools' }],
          description: [{ key: 'en', value: 'Created from GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION E2E' }],
          state: true
        }).then(function (type) {
          return app.$store.dispatch('security/menu/createMenu', {
            title: [{ key: 'en', value: 'Custom GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Tool' }],
            description: [{ key: 'en', value: 'Custom menu created from GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION' }],
            path: '/custom/graduationsystemusingfacerecognition-tool',
        ...
        browser.assert.ok((result.value.types || []).some(function (name) { return name === 'Custom GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION Tools' }), 'created type remains visible')
        browser.assert.ok((result.value.menus || []).some(function (path) { return path === '/custom/graduationsystemusingfacerecognition-tool' }), 'created menu remains visible')
      })
      .end()
  }
}
