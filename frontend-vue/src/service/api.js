...
const defaultApiBaseUrl = typeof window !== 'undefined' && window.location && window.location.hostname
  ? `http://${window.location.hostname}:8206`
  : 'http://127.0.0.1:8206';
...
export default {
  ...
  accounts(method, data) {
    switch (method) {
      ...
      case 'remove-graduationsystemusingfacerecognition-access':
        return instance.delete(`/api/v1/accounts/${data && (data.id || data._id)}/graduationsystemusingfacerecognition-access`);
      case 'lifecycle':
  ...
  graduationsystemusingfacerecognitionDocuments(method, data) {
    switch (method) {
      case 'stats':
        return instance.get('/api/v1/graduationsystemusingfacerecognition/documents/stats', { params: data || {} });
      case 'list':
        return instance.get('/api/v1/graduationsystemusingfacerecognition/documents', { params: data || {} });
      case 'create':
        return instance.post('/api/v1/graduationsystemusingfacerecognition/documents', data || {});
      case 'update':
        return instance.put(`/api/v1/graduationsystemusingfacerecognition/documents/${data && (data.id || data._id)}`, data || {});
      case 'delete':
        return instance.delete(`/api/v1/graduationsystemusingfacerecognition/documents/${data && (data.id || data._id)}`);
      case 'seed-demo':
        return instance.post('/api/v1/graduationsystemusingfacerecognition/documents/seed-demo', data || {});
      default:
        break;
    }
  },
  ...
  graduateRegistrations(method, data) {
    switch (method) {
      ...
      case 'list':
        return instance.get('/api/v1/graduationsystemusingfacerecognition/registrations', { params: data || {} });
      case 'create':
