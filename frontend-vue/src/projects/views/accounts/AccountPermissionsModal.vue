...
export default {
  ...
  computed: {
    ...
    sessionRows () {
      return (Array.isArray(this.sessions) ? this.sessions : []).map(item => ({
        ...
        system: item && item.system ? String(item.system) : 'graduationsystemusingfacerecognition',
  ...
  methods: {
    getSystemLabel (system) {
      const normalized = system ? String(system).trim() : 'graduationsystemusingfacerecognition'
      return normalized ? normalized.toUpperCase() : 'GRADUATIONSYSTEMUSINGFACERECOGNITIONGRADUATIONSYSTEMUSINGFACERECOGNITION'
    },
    grantLabels (item) {
      const labels = []
